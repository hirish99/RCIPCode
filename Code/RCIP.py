...
def old_rcip_problem_KcircR(npan, nsub):
    n = 16
    T, W, _ = sps.legendre(n).weights.T
...
def get_fine_density_ellipse_rcip_improved(npan, nsub, test_charge, target_complex):
    T, W, _ = sps.legendre(n).weights.T
    IP, IPW = IPinit(T,  W)
...
def teardrop_rcip_improved_KcircR(npan, nsub):
    T, W, _ = sps.legendre(n).weights.T
    IP, IPW = IPinit(T,  W)
...
def teardrop_rcip_improved_density(npan, nsub):
    T, W, _ = sps.legendre(n).weights.T
    IP, IPW = IPinit(T,  W)
